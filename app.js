...
const IMGBB_API_KEY = "f680d20522f2abe01c499344414be811";
...
function openForm(id){
    ...
    <div class="field">
      ...
      <div class="imgpicker">
        ...
        <div style="flex:1;display:flex;flex-direction:column;gap:8px;">
          <label class="btnfile" style="text-align:center;">📷 Subir foto
            <input type="file" id="fImageFile" accept="image/*" style="display:none;">
          </label>
          <input type="text" id="fImageUrl" placeholder="o pega el link de una foto..." value="${escapeHtml(tempImage)}"
            style="padding:9px 12px;border-radius:10px;border:1px solid var(--line);background:var(--panel);color:var(--text);font-size:13px;">
        </div>
      </div>
    </div>
  ...
  overlay.querySelector("#fImageFile").onchange = (e)=>{
    ...
    compressImage(file, async (dataUrl)=>{
      ...
      if(!url){
        showToast("No se pudo subir la imagen. Intenta de nuevo.");
        return;
      }
      tempImage = url;
      overlay.querySelector("#fImageUrl").value = url;
      overlay.querySelector("#imgPrev").innerHTML = `<img src="${escapeHtml(url)}">`;
